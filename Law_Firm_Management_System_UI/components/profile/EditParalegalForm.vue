    ...
    <v-card-text class="px-8 py-4 text-body-1">
      <v-row>
        <v-col cols="2" class="d-flex align-center">
          <v-label class="fw-bold">Assigned Partner</v-label>
        </v-col>
        <v-col cols="4">
          <v-card-subtitle class="p-0 text-wrap">{{ roleInfo.assignedPartner ?? '-' }}</v-card-subtitle>
        </v-col>
      </v-row>
      <v-row>
        <v-col cols="2" class="d-flex align-center">
...
const editRole = handleSubmit(async(values, { resetForm }) => {
  try {
    const result = await fetchData.$put("/User/Me/Info/Role/Paralegal", values)
    if (!result.error) {
      isEdit.value = !isEdit.value
      ...
      ElNotification.error({ message: "Your paralegal information has been edited unsuccessfully" })
    }
