  ...
  <form @submit.prevent="editRole">
    <v-card-text class="px-8 py-4 text-body-1">
      <v-row>
        ...
        <v-col cols="4">
          <v-card-subtitle class="p-0 text-wrap" v-if="!isEdit">{{ roleInfo.fullName }}</v-card-subtitle>
          <v-text-field
            variant="outlined"
            density="compact"
            :error-messages="editRoleDetails.fullName.errorMessage"
            v-model="editRoleDetails.fullName.value"
            hide-details="auto"
            v-else
          />
        </v-col>
      ...
      <v-row>
        <v-col cols="2" class="d-flex align-center">
          <v-label class="fw-bold">Email</v-label>
        </v-col>
        <v-col>
          <v-card-subtitle class="p-0 text-wrap" v-if="!isEdit">{{ roleInfo.email ?? '-' }}</v-card-subtitle>
          <v-text-field
            variant="outlined"
            density="compact"
            :error-messages="editRoleDetails.email.errorMessage"
            v-model="editRoleDetails.email.value"
            hide-details="auto"
            v-else
          />
        </v-col>
...
const { handleSubmit } = useForm({
  initialValues: {
    fullName: props.roleInfo.fullName,
    phoneNum: props.roleInfo.phoneNum,
    email: props.roleInfo.email,
...
const editRoleCancel = () => {
  ...
  editRoleDetails.value.fullName.resetField()
  editRoleDetails.value.phoneNum.resetField()
  editRoleDetails.value.email.resetField()
  editRoleDetails.value.address.resetField()
}
const editRole = handleSubmit(async(values, { resetForm }) => {
  try {
    const result = await fetchData.$put("/User/Me/Info/Role/Client", values)
    if (!result.error) {
      isEdit.value = !isEdit.value
      ...
      ElNotification.error({ message: "Your client information has been edited unsuccessfully" })
    }
